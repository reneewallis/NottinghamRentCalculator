...
describe("calcDropdownMinWidth tests", () => {
    ...
    test("default dropdown, max label length is negative, should throw error", ()=>{
        expect(() => {calcDropdownMinWidth({dropdownStyle:"DEFAULT", maxLabelLength:-5});}).toThrow(new Error(`max label length "-5" cannot be negative`));
    });
    ...
    test("small dropdown test, longest word is negative, should through error", () => {
        expect(()=>{calcDropdownMinWidth({dropdownStyle:"SMALL",maxLabelLength:5, longestWordLength:-2})}).toThrow( new Error(`longest word length "-2" cannot be negative`));
    });
});