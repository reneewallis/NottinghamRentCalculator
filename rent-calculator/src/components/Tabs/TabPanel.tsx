...
function calculateShortfall(benefitType: BenefitType, benefitValue: string, weeklyRent: string, fourWeeklyRent: string, monthlyRent: string): ShortfallState {
    const benefitValueIsValid = benefitValue !== '' && isValidNumberEntry(benefitValue);
    ...
    if (benefitValueIsValid && weeklyRent !== '' && fourWeeklyRent !== '' && monthlyRent !== ''){
        ...
        switch (benefitType){
            case BenefitType.HOUSING_BENEFIT:{
                ...
                monthlyShortfall = (ceil2DP((benefitAmount * 13) / 12) - (+monthlyRent)).toFixed(2);

                break;
            }

            case BenefitType.UNIVERSAL_CREDIT:{
                weeklyShortfall = (ceil2DP((benefitAmount*12) / 52) - (+weeklyRent)).toFixed(2);
                fourWeeklyShortfall = (ceil2DP((benefitAmount*12) / 13) - (+fourWeeklyRent)).toFixed(2);
...
function calculateTotalInstallments(startingBalance: string, defaultAmount: string): number{
    if (startingBalance === '' || defaultAmount === '' || !isValidNumberEntry(startingBalance) || !isValidNumberEntry(defaultAmount)){
        return -1;
    }
...
function calculateInstallment(startDate: Dayjs, forecastDate: Dayjs, paymentFrequency:InstallmentFrequency): number{
...
function calculateForecastDate(startDate:Dayjs, minForecastDate:Dayjs, paymentFrequency:InstallmentFrequency, installmentNumber: number):Dayjs | null {
    ...
    return forecastDate.isBefore(minForecastDate) ? minForecastDate : forecastDate;
}


function calculateForecastPaid(installmentNumber: number, defaultAmount:string, startingBalance:string): string{
    if (defaultAmount === '' || startingBalance === '' || !isValidNumberEntry(defaultAmount) || !isValidNumberEntry(startingBalance)){
        return '';
    }
...
function calculateBalanceRemaining(startingBalance:string, totalPaid:string): string{
    if (startingBalance === '' || totalPaid === '' || !isValidNumberEntry(startingBalance) || !isValidNumberEntry(totalPaid)){
        return '';
    }
...
function calculateForecast(startingBalance: string, startDate: Dayjs | null, startDateIsValid: boolean, forecast:ForecastState): ForecastState{
    ...
    if (totalInstallments !== -1) {
        if (startDate && startDateIsValid && forecast.forecastDate && forecast.forecastDateIsValid && forecast.paymentFrequency !== InstallmentFrequency.UNSELECTED){
            newInstallmentNumber = calculateInstallment(startDate, forecast.forecastDate, forecast.paymentFrequency);
        }
        ...
        newInstallmentNumber = newInstallmentNumber > totalInstallments? totalInstallments: newInstallmentNumber;
        
        forecastPaid = calculateForecastPaid(newInstallmentNumber, forecast.defaultAmount, startingBalance);
...
function reducer(state:CalculatorState, action:CalculatorAction): CalculatorState{
    ...
    switch (action.type){
        ...
        case CalculatorActions.CHANGE_INSTALLMENT_NUMBER:{
            ...
            if (balance.startDate && balance.startDateIsValid){
                const forecastDate = calculateForecastDate(balance.startDate, forecast.minForecastDate, forecast.paymentFrequency, forecast.installmentNumber);
                if (forecastDate){
...
function TabPanel({today}:PanelProps){
    ...
    const calculatorBoxes : ({name:string} & CalculatorBoxProps)[] = [
        {
            ...
            dropDownProps: {
                ...
                items: [
                    {label: RentFrequency.WEEKLY, onClick: () => {calculatorDispatch({type: CalculatorActions.CHANGE_RENT_FREQUENCY, newRentFrequency: RentFrequency.WEEKLY})}}, 
                    {label: RentFrequency.FOUR_WEEKLY, onClick: () => {calculatorDispatch({type: CalculatorActions.CHANGE_RENT_FREQUENCY, newRentFrequency: RentFrequency.FOUR_WEEKLY})}}, 
                    {label: RentFrequency.MONTHLY, onClick: () => {calculatorDispatch({type: CalculatorActions.CHANGE_RENT_FREQUENCY, newRentFrequency: RentFrequency.MONTHLY})}}
                ],
            ...
            inputTextBoxProps: {
                ...
                readOnly: false,
                onChange: e => {calculatorDispatch({type: CalculatorActions.CALCULATE_RENT, amount: e.target.value})},
                valid: calculatorState.rentAmountIsValid,
            },
            resultTextBoxes: [
                {label: "Weekly Rent", text: calculatorState.weeklyRent, readOnly: true},
                {label: "4-Weekly Rent", text: calculatorState.fourWeeklyRent, readOnly: true},
                {label: "Monthly Rent", text: calculatorState.monthlyRent, readOnly:true}
            ]
        },
        {
            name: "shortfall",
            dropDownProps: {
                ...
                items: [
                    {label: BenefitType.UNIVERSAL_CREDIT, onClick: () => {calculatorDispatch({type: CalculatorActions.CHANGE_BENEFIT_TYPE, newBenefitType: BenefitType.UNIVERSAL_CREDIT})}}, 
                    {label: BenefitType.HOUSING_BENEFIT, onClick: () => {calculatorDispatch({type: CalculatorActions.CHANGE_BENEFIT_TYPE, newBenefitType: BenefitType.HOUSING_BENEFIT})}},
                ],
            ...
            inputTextBoxProps: {
                ...
                readOnly: false,
                onChange: e => {calculatorDispatch({type: CalculatorActions.CALCULATE_SHORTFALL, amount: e.target.value})},
                valid: calculatorState.benefitAmountIsValid
            },
            resultTextBoxes: [
                {label: "Weekly Shortfall", text: calculatorState.weeklyShortfall, readOnly: true},
                {label: "4-Weekly Shortfall", text: calculatorState.fourWeeklyShortfall, readOnly: true},
                {label: "Monthly Shortfall", text: calculatorState.monthlyShortfall, readOnly:true}
            ]
        }
    ];
    ...
    const minDropdownWidth = (Math.max(0, ...calculatorBoxes.map(box => Math.max(box.dropDownProps.label.length, ...box.dropDownProps.items.map(item => item.label.length))))) * 1.15
    return (<div className="mt-6 px-6 w-full">
                <div className="grid grid-cols-[repeat(2,minmax(35.5rem,1fr))] grid-flow-row-dense overflow-x-auto justify-start items-center gap-12 pb-5">
                    {calculatorBoxes.map(box => 
                        ...
                        <CalculatorBox
                            dropDownProps={{...box.dropDownProps, minWidth: minDropdownWidth}} 
                            inputTextBoxProps={box.inputTextBoxProps} 
                            resultTextBoxes={box.resultTextBoxes}></CalculatorBox>
                        </div>
                    )}
